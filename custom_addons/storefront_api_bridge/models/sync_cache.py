...
class StorefrontWebhookEvent(models.Model):
    ...
    def _cron_process_pending(self, limit=40):
        ...
        events = self.sudo().search(
            [("state", "=", "pending")], limit=limit, order="received_at, id",
        )
        inventory_events = events.filtered(lambda event: event.event_type == "inventory.updated")
        if inventory_events:
            inventory_events.write({"state": "processing"})
            try:
    ...
    def _sync_product(self):
        ...
        product.with_context(shop_api_skip_event=True, tracking_disable=True).write(field_values)
        if self.event_type == "product.image.updated":
    ...
    def _sync_product_images(self, product, images):
        ...
        for image in images:
            ...
            if image.get("kind") == "cover":
                product.with_context(shop_api_skip_event=True, tracking_disable=True).image_1920 = binary
                continue
            gallery = Image.with_context(active_test=False).search([
            ...
            if gallery:
                ...
                Image.create({"shop_api_uuid": image_id, **values})
